...
function searchForSimilarWords(
  node: Node,
  word: string,
  options: { maxDistance: number; maxResults: number } = {
    maxDistance: 3,
  ...
  comparedWord = '',
  table = initializeEditDistanceMatrix(word.length),
): any {
  let results: any = [];
  for (let key of Object.keys(node.states)) {
    ...
    if (
      node.nodeOf(key).final &&
      editDistanceValue <= options.maxDistance
    ) {
...
function editDistanceOf(data: any) {
  ...
  let inequalityPunishment =
    data.word[firstIndex - 1] === data.comparedWord[secondIndex - 1]
      ? 0
      : 1;

  return Math.min(
    table[firstIndex][secondIndex - 1] + 1,
    table[firstIndex - 1][secondIndex] + 1,
    table[firstIndex - 1][secondIndex - 1] + inequalityPunishment,
  );
}

function initializeEditDistanceMatrix(len: number) {
  let matrix = [];
